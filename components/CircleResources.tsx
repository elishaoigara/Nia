...
export default function CircleResources({ circleId, userId, isOwner }: Props) {
  ...
  return (
    <section className="card space-y-4" aria-labelledby="circle-resources-heading">
      <div className="circle-resources-header">
        <div className="circle-resources-copy"><p className="text-[11px] font-extrabold uppercase tracking-[0.08em]" style={{ color: 'var(--nia-violet)' }}>Shared shelf</p><h2 id="circle-resources-heading" className="mt-1 text-lg font-extrabold">Resources for the journey</h2><p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>Keep useful links, tools, opportunities, and events together.</p></div>
        {isOwner && <button type="button" className="circle-resources-add btn-ghost flex items-center gap-1.5 text-xs" onClick={() => setOpen(value => !value)}>{open ? <X size={14} /> : <Plus size={14} />} {open ? 'Close' : 'Add resource'}</button>}
      </div>

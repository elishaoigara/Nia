...
export default function CreatePost({
  ...
  function handleImage(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Clear voice/poll if image selected
    setVoiceBlob(null)
  }
  ...
  async function handlePost() {
    ...
    try {
      ...
      // Create poll
      if (hasPoll && post) {
        const validOptions = pollOptions.filter((o) =>
          o.trim()
        )

        const endsAt = new Date(
          Date.now() +
            parseInt(pollDuration) * 60 * 60 * 1000
        ).toISOString()
  ...
  return (
    <div
      ...
      style={
        focused
          ? {
              border: '1.5px solid var(--nia-violet)',
              boxShadow:
    ...
    >
      ...
      {showPoll && (
        <div
          className="mx-4 mb-3 p-4 rounded-2xl space-y-3"
      ...
      >
        ...
        <div className="relative">
          ...
          {showLangPicker && (
            <div
              className="absolute bottom-full mb-2 left-0 z-50 rounded-2xl p-2 shadow-xl grid grid-cols-2 gap-1"
              style={{

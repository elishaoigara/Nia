...
import {
  ImagePlus, X, Loader2, Sparkles, BarChart2, Plus, Trash2,
} from 'lucide-react'
  ...
  async function handlePost() {
    ...
    try {
      if (voiceBlob) {
        ...
        if (uploadError) { setError('Voice upload failed.'); setLoading(false); return }
        const { data } = supabase.storage.from('post-media').getPublicUrl(path)
        media_url = data.publicUrl; media_type = 'audio'
      } else if (image) {
        const ext = image.name.split('.').pop()
        const path = `${userId}/${Date.now()}.${ext}`
        const { error: uploadError } = await supabase.storage.from('post-media').upload(path, image)
      ...
      const { data: post, error: postError } = await supabase
        ...
        .insert({
          ...
          media_url, media_type, is_anonymous: isAnonymous, language,
        })
        .select().single()
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Post button — always at far right */}
        <button
          onClick={handlePost}

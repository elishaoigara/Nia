  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-2xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg,rgba(255,107,107,0.15),rgba(168,85,247,0.15))' }}>
          ...
          {notifications.map((n: any, i: number) => {
            ...
            return (
              <div
                ...
                style={!n.is_read ? { borderLeft: '3px solid var(--nia-violet)', background: 'linear-gradient(135deg,rgba(168,85,247,0.04),transparent)' } : {}}
              >
                {/* Type icon */}
                <div className="w-9 h-9 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: meta.bg }}>
                  <Icon size={16} style={{ color: meta.color }} fill={n.type === 'like' ? meta.color : 'none'} />
                </div>

                {/* Actor avatar */}
                <Link href={`/profile/${n.actor?.id}`} className="flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm">
                    <Link href={`/profile/${n.actor?.id}`} className="font-bold hover:underline">@{n.actor?.username}</Link>
                    {' '}
                ...
                {!n.is_read && <div className="w-2 h-2 rounded-full mt-1.5 flex-shrink-0" style={{ background: 'var(--nia-violet)' }} />}
              </div>
            )
          })}
        </div>

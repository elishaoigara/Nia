...
export default function MarketplaceClient({
  ...
  async function createListing() {
    ...
    const { error: e } = await supabase.from('marketplace_listings').insert({
      user_id: currentUserId, title: title.trim(),
      description: description.trim() || null,
      price: parseFloat(price), category, condition, image_url, status: 'active',
    })
    if (e) { setError(e.message); setPosting(false); return }
    setShowCreate(false); setTitle(''); setDescription(''); setPrice('')
    setCategory('other'); setCondition('good'); setImageFile(null); setImagePreview(null)
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ background: 'linear-gradient(135deg,rgba(255,217,61,0.2),rgba(255,142,83,0.2))' }}
          >
            🛍️
          </div>
          <div>
        ...
        <button
          ...
          style={{ borderRadius: '14px', minHeight: '40px' }}
        >
          <Plus size={16} /> Sell
        </button>
      </div>
      ...
      {/* Listings grid — 1 col on very small, 2 col on xs+ */}
      {filtered.length === 0 ? (
          ...
          {filtered.map(item => (
            <div key={item.id} className="card card-hover overflow-hidden flex flex-col">
              ...
              <div className="p-3 space-y-1.5 flex flex-col flex-1">
                ...
                <p className="font-extrabold text-sm" style={{ color: 'var(--nia-mint)' }}>
                ...
                {item.user_id !== currentUserId && (
                  <Link
                    ...
                    className="mt-auto flex items-center justify-center gap-1.5 w-full py-2 rounded-xl text-xs font-bold transition-all active:scale-95"
                    style={{
                      background:
                        'linear-gradient(135deg,rgba(107,203,119,0.15),rgba(78,205,196,0.15))',
                      color: 'var(--nia-mint)',
                    }}
                  >
      ...
      {showCreate && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-t-[28px] sm:rounded-[28px] p-6 space-y-4 anim-up max-h-[90dvh] overflow-y-auto"
          ...
          >
            ...
            <div
              ...
              className="w-full h-32 rounded-2xl flex items-center justify-center cursor-pointer transition-all overflow-hidden"
              style={{
                border: '2px dashed var(--border)',
                background: imagePreview ? undefined : 'var(--surface-2)',
              }}
            >
              {imagePreview ? (
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="text-xs font-bold mb-1 block" style={{ color: 'var(--text-tertiary)' }}>
                  Price (KES)
                </label>
                <input
                  type="number"
